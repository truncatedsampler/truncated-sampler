...
class SEP_Parameters:
    ...
    def __init__(self, target_bitsec:int, n:int, d:int, l:int, m:int, q_start:int):
        """
        ...
        - output:
            SEP_Parameters object with the following attributes
                [+] Parameters
                    ...
                    (flt)   s_11    -- Preimage sampling width of v_11
                    (flt)   s_12    -- Preimage sampling width of v_12
                    (flt)   s_2     -- Preimage sampling width of v_2, v_3
                    ...
                    (flt)   B_11_prime -- Verification bound for v_11 - r1 (hiding case)
                    (flt)   B_11_prime_s -- Verification bound for v_11 - r1 (hiding case) squared
                    (flt)   B_12    -- Verification bound for v_12
                    (flt)   B_12_s  -- Verification bound for v_12 squared
                    (flt)   B_12_prime -- Verification bound for v_12 - r2 (hiding case)
                    (flt)   B_12_prime_s -- Verification bound for v_12 - r2 (hiding case) squared
        ...
        # Computing Gaussian widths for truncated sampler
        ...
        self.s_11 = max(sqrt(pi / log(2)) * (norm_Si_m + sqrt(self.d * self.n)), self.s_11_no_rej)
        self.s_3 = max(sqrt(pi / log(2)) * (norm_Si_m + sqrt(self.d * self.n)), self.s_3_no_rej)
        ...
        self.B_11_s = floor(c_star(self.d * self.n, self.sec + 3) ** 2 * self.s_11 ** 2 * (self.d * self.n))
        self.B_11 = sqrt(self.B_11_s)
        self.B_12_s = floor(c_star(self.d * self.n, self.sec + 3) ** 2 * self.s_3 ** 2 * (self.d * self.n))
        ...
        self.B_2_s = floor(c_star((self.k-self.l) * self.d * self.n, self.sec + 3) ** 2 * self.s_4 ** 2 * ((self.k-self.l) * self.d * self.n))
        self.B_2 = sqrt(self.B_2_s)

        self.B_3_s = floor(c_star((self.k-self.l) * self.n, self.sec + 3) ** 2 * self.s_4 ** 2 * ((self.k-self.l) * self.n))
        ...
        # M-SIS hardness
        self.msis_beta_I = sqrt(floor( (sqrt(self.B_11_prime_s + self.B_12_prime_s) + sqrt(self.d * self.n) * self.B_2) ** 2 + self.B_3_s + self.m * self.n + 1 ))
        res = estimate_SIS(n = self.n * self.d,
                        m = self.n * (2 * self.d + self.k - self.l + self.m + 1),
                        q = self.q,
        ...
        self.msis_beta_II = sqrt(floor( (2 * sqrt(self.B_11_prime_s + self.B_12_prime_s) + sqrt(self.d * self.n) * sqrt(4*self.B_2_s + self.n*self.m)) ** 2 + 4*self.B_3_s ))
        res = estimate_SIS(n = self.n * self.d,
        ...
        Ek = (self.k-self.l) * mlwe_S_hardness_bound
        ...
        self.eps_II = C * self.Q * msis_II_hardness_bound
        for _ in range(self.d):
            self.eps_I = h(self.eps_I)
            self.eps_II = h(self.eps_II)
        self.eps_II = Em + 4*self.M_11*self.M_12*C**2*(1+eps)/(1-eps)*self.eps_II
    ...
    def __repr__(self):
        ...
        tmp += '| {:60s} | {:^10s} | {:<20.5f} |\n'.format('Preimage sampling width v_2, v_3', 's_4', self.s_4)
        ...
        tmp += '| {:60s} | {:^10s} | {:<20.5f} |\n'.format('Signature verification bound 11 (hiding)', 'B_11', self.B_11_prime)
        tmp += '| {:60s} | {:^10s} | {:<20.5f} |\n'.format('Signature verification bound 12 (hiding)', 'B_12', self.B_12_prime)
        ...
        tmp += 100 * '=' + '\n'
        tmp += '{:-^100s}'.format('Unforgeability') + '\n'
        tmp += '| {:60s} | {:^10s} | {:<20.5f} |\n'.format('Type-I Security', '-log(eps_I)', -log2(self.eps_I))
        tmp += '| {:60s} | {:^10s} | {:<20.5f} |\n'.format('Type-II Security', '-log(eps_II)', -log2(self.eps_II))
...
class Show_ZKP_Parameters:
    ...
    def __init__(self, target_bitsec:int, n:int, d:int, m_2:int, q_1_start:int, n_attr:int, gamma:int, D:int, sig, bimodal=True, compression=True, garbage=True):
        ...
        # Witness dimension
        self.m_1 = sig.d * self.k + 1 # v_11 (+ a_11 for four-square norm proof)
        self.m_1 += sig.d * self.k + 1 # v_12 (+ a_12 for four-square norm proof)
        self.m_1 += sig.d * (sig.k - sig.l) * self.k + 1 # v_2 (+ a_2 for four-square norm proof)
        self.m_1 += (sig.k - sig.l) * self.k + 1 # v_3 (+ a_3 for four-square norm proof)
        self.m_1 += self.k # tag
        self.m_1 += (sig.m - self.n_attr) * self.k # msg (include usk)
        if bimodal:
            self.m_1 += 1 # bimodal bit
        ...
        # Bound on Euclidean norm of the witness
        self.bound_witness =  sqrt(sig.B_11_prime_s + sig.B_12_prime_s + sig.B_2_s + sig.B_3_s + sig.w + sig.n * (sig.m - self.n_attr) + (1 if bimodal else 0))
        ...
        self.B_256 = sqrt(self.B_256_s)
        self.B_arp = self.B_256 * 2/sqrt(26)
        self.cond_1_bound = max(sig.B_11_prime_s, sig.B_12_prime_s, sig.B_2_s, sig.B_3_s, sig.w, 1) # Lower bound -q < -B^2
        self.cond_2_bound = 41 * self.n * self.m_1 * self.B_arp # Condition for modular JL bound for ARP bound
        self.cond_3_bound = self.B_arp**2 - min(sig.B_11_prime_s, sig.B_12_prime_s, sig.B_2_s, sig.B_3_s, sig.w, 1) # Upper bound (2/root(26) * B_e)^2 - B^2 < q
        self.cond_4_bound = sig.w + sqrt(sig.w*self.n*self.k) # bound for proving tag binary
        self.cond_5_bound = self.B_arp**2 + self.B_arp * sqrt(self.n*self.k*(sig.m-self.n_attr)) # bound for proving msg binary
        x = (self.q_1*pow(self.q_1, -1, self.q_2) - self.q_2*pow(self.q_2, -1, self.q_1)) % self.q
        if x > self.q/2: x -= self.q
...
sig_pms = SEP_Parameters(target_bitsec=128, n=256, d=4, l=2, m=10, q_start=ceil(2 ** 18.7))
...
show_zkp_pms = Show_ZKP_Parameters(target_bitsec=128, n=64, d=23, m_2=74, q_1_start=ceil(2 ** 39.8), n_attr=0, gamma=2**30, D=22, sig=sig_pms, bimodal=ZK_OPTIMIZATIONS, compression=ZK_OPTIMIZATIONS, garbage=ZK_OPTIMIZATIONS)
