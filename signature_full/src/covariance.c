...
void compute_covariance(poly_real_mat_2d_2d S, const poly_q_mat_d_d RRstar[2][2]) {
  ...
  // convert RRstar to poly_real_mat_2d_2d and write in S and multiply by -1/(1/sG^2 - 1/s4^2)) and add s1^2.I_2d (RRstar needs to be read in centered representation!)
  for (i = 0; i < 2*PARAM_D; i++) {
    for (j = 0; j < 2*PARAM_D; j++) {
      ...
      poly_real_mul_scalar(S->rows[i]->entries[j], S->rows[i]->entries[j], PARAM_SGINVSQ_S2INVSQ);
    }
...
**************************************************/
int sk_sq_spectral_norm(poly_q_mat_d_d RRstar[2][2], const poly_q_mat_d_d R[2][PARAM_K]) {
  size_t i,j,k;
