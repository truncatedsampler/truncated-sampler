...
void sign(sig_t *sig, uint8_t state[STATE_BYTES], const sk_t *sk, const pk_t *pk, const uint8_t msg[PARAM_N/8]) {
  ...
  poly_q_mat_d_k_l_uniform(A3, pk->seed, DOMAIN_SEPARATOR_A3);
...
reject_signature:
  ...
  poly_q_vec_k_l_sample_gaussian_s4(sig->v3); // probabilistic

  // compute u + m.d - A3.v3 (d used as temp variable)
  poly_q_mat_d_k_l_mul_vec_k_l(d, A3, sig->v3);
  poly_q_vec_d_sub(u, u, d);

  // call SamplePre, output in v1, sig->v2
  truncated_sampler(v11, sig->v12, sig->v2, sk->R, A, pk->B, u, sig->tag, sk->S); // probabilistic
  ...
  norm2sq_v3 = poly_q_vec_k_l_norm2(sig->v3);
...
int verify(const sig_t *sig, const uint8_t msg[PARAM_N/8], const pk_t *pk) {
  ...
  poly_q_mat_d_k_l_mul_vec_k_l(u, A3, sig->v3);
